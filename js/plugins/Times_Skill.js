 ...
 (function() {
	...
	Game_BattlerBase.prototype.recoverTimes_Skill = function() {
		for(key in this._times_skill){
			this._times_skill[key][0] = this._times_skill[key][1];
		};
	};
	// スキル使用コストの支払い可能判定
	...
	Window_SkillList.prototype.includes = function(item) {
		if (item.meta.times_Skill && this._actor._times_skill[item.id] === undefined) {
			this._actor._times_skill[item.id] = [item.mpCost, item.mpCost]
		}
	...
	Window_SkillList.prototype.drawSkillCost = function(skill, x, y, width) {
		if (skill.meta.times_Skill) {
			...
			this.drawText(this._actor._times_skill[skill.id][0], x, y, width, 'right');
		} else {
